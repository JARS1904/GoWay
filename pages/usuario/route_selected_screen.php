...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Listener del toggle "Es trayecto de regreso"
            // Se registra aquí dentro de DOMContentLoaded porque el elemento
            // está en el HTML del modal, que se encuentra DESPUÉS del bloque <script>.
            ...
            if (toggleRetorno) {
                toggleRetorno.addEventListener('change', function() {
                    if (!window.currentAsignacionData) {
                        if (this.checked) {
                            showToast('Busca un vehículo primero');
                            this.checked = false;
                        }
                        return;
                    }
        ...
        function searchReportAssignment() {
            ...
            if (!placa) {
                showToast('Ingresa una placa para buscar');
                return;
            }
            ...
            fetch(`../../api/reportes_api.php?action=get_assignment_data&placa=${encodeURIComponent(placa)}`)
                ...
                .then(data => {
                    ...
                    if (data.success && data.data) {
                        ...
                        showToast('Asignación encontrada');
                    } else {
                        document.getElementById('reportAssignmentCard').style.display = 'none';
                        document.getElementById('reportIdAsignacion').value = '';
                        showToast(data.error || 'No se encontró asignación');
                    }
                })
                .catch(err => {
                    btn.innerText = originalText;
                    showToast('Error al buscar asignación');
                });
        }
        ...
        function submitReport() {
            ...
            if (!idAsignacion) {
                showToast('Debes buscar y seleccionar un vehículo primero');
                return;
            }
            ...
            .then(data => {
                if (data.success || data.id_reporte) {
                    ...
                    showToast(data.error || data.message || 'Error al enviar reporte');
                }
            })
            .catch(err => {
        ...
        // (El listener de reportEsRetorno se registra dentro de DOMContentLoaded arriba)
    ...
    <script>
        ...
        async function loadFavoritesPanel() {
            ...
            try {
                const res = await fetch(`${FAVORITES_URL}?action=get_favorites&id_usuario=${ID_USUARIO}`, {
                    headers: { 'Accept': 'application/json' }
                });
        ...
        async function selectFavoriteRoute(routeId) {
            ...
            let route = routes.find(r => r.id_ruta === routeId);

            // 2. Buscar en el mapa cacheado del panel de favoritas
            if (!route && window.favoritesDataMap[routeId]) {
            ...
            if (!route) {
                try {
                    const res = await fetch(`${API_URL}?action=route_detail&id_ruta=${routeId}`, {
                        headers: { 'Accept': 'application/json' }
                    });
                    if (res.ok) {
                        const data = await res.json();
                        if (Array.isArray(data) && data.length > 0) {
                            route = processRoutes(data)[0];
                        }
                    }
            ...
            if (route) {
                ...
                showToast('No se pudo cargar el detalle de la ruta');
            }
        }
        ...
        async function removeFavoriteFromPanel(routeId, btnElement) {
            try {
                const response = await fetch(FAVORITES_URL, {
                    ...
                    body: JSON.stringify({ action: 'remove_favorite', id_usuario: ID_USUARIO, id_ruta: routeId })
                });
                const data = await response.json();
                if (data.success) {
                    favorites.delete(routeId);
                    delete window.favoritesDataMap[routeId];
                    ...
                    showToast('Error al eliminar favorita');
                }
            } catch(e) {
                showToast('Error de conexión');
            }
    ...
    <!-- estilos del panel de favoritas ahora en route_selected_screen.css -->
    <?php endif; ?>
